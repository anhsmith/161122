...
read.csv("http://www.massey.ac.nz/~jcmarsha/227215/data/donkey.csv") %>% 
...
```{r, fig.align='center', fig.width=6, fig.height=4, out.height="70%", echo=FALSE}
par(mar=c(2,2,2,2))
plot(mod, which=1, pch=19, col="#00000040")
```
...
which implies a power law between `Bodywt` and `Heartgirth` with a multiplicative error term $e^\varepsilon$. $e^\varepsilon$ follows a log-normal distribution. 
...
$$
y = \alpha + \beta \log(x) +\varepsilon \text{    and    } \log(y) = \alpha + \beta x + \varepsilon
$$
...
A bit calculus shows that ${(y^{\lambda}-1)}/{\lambda}\to \log(y)$ if $\lambda\to 0$.
