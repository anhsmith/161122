...
output:
  xaringan::moon_reader:
    css: [default, default-fonts, "custom.css", "adam.css"]
...
- A good scientists is sceptical, only believing in something if there is empirical evidence to support it.  
...
We are 95% confident that the true **average** body weight of donkeys with a heart girth of 120 is between  `r predict(mod, new.data, interval="confidence")[1,2]` and `r predict(mod, new.data, interval="confidence")[1,3]`. 
...
We estimate that 95% of donkeys with a heart girth of 120 will have a body weight between  `r predict(mod, new.data, interval="prediction")[1,2]` and `r predict(mod, new.data, interval="prediction")[1,3]`. Note we have not used the word 'mean' or 'average' here, because this is about the spread of values around the mean.
...
legend("bottomright", border = NA, 
       ...
       legend=c("Regression line", "Confidence interval", "Prediction interval"), bty="n")
```
...
## Model outputs with transformation 
...
## Model outputs without transformation 
...
## Comparing two models via plots
