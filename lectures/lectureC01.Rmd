...
g5 <- ggplot(donkey, aes(x=Sex, y=Bodywt)) + geom_boxplot(col=col_points) 
...
```{r echo=FALSE, fig.width=8, fig.height=6, out.width="65%"}
model_wt_hg <- lm(Bodywt ~ Heartgirth, data=donkey)
ab <- coef(model_wt_hg)
p <- predict(model_wt_hg,
          newdata=data.frame(Heartgirth=70:150),
          interval='prediction')
g1 <- ggplot(donkey, aes(x=Heartgirth, y=Bodywt)) +
  geom_point(col=col_points) +
...
```{r echo=FALSE, fig.width=8, fig.height=6, out.width="65%"}
g3 <- g2 +
  annotate('line', x=70:150, y = p[,2], col = "grey") +
  annotate('line', x=70:150, y = p[,3], col = "grey") +
  # geom_smooth(method="lm", se=T, level= col = "black") + 
...
simple_plot <- function(x,y) {
  ...
  plot(y ~ x, col=col_points, xlim=c(0, max(x)+0.2), ylim=c(0, max(y)+0.2), 
...
text(3.8, yv[1]-0.5, expression(beta==over(rise,run)))
...
You will never observe the true noises! But all the randomness comes from them. 
...
shape::Arrows(x_i, y_h, x_i, y_i, col="grey20", arr.adj = 1, arr.length = 0.2, arr.width= 0.2, arr.type="triangle", lwd=1)
lines(c(-4, x_i), c(y_h, y_h), lty='dotted')
axis(2, at=c(y_h, y_i), line=0, labels=expression(hat(y)[8], y[8]), las=1)
points(x_i, y_h, pch=16, cex=.7, col="grey20")
...
simple_plot(x,y)
points(x, y_hat, pch=16, cex=.7, col="grey20")
shape::Arrows(x, y, x, y_hat, col="grey20", arr.length=0, lwd=1)
legend(legend=c("Observed data", "Predicted values of y","Residuals (unexplained variation in y)"), x = "topleft", bty="n",
       pch=c(19,16,124), pt.cex=c(1,.7, 0.7), col=c(col_points,"grey20","grey20"), cex=.7)
